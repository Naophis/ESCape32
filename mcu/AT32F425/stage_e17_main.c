/*
 * ESCape32 AT32F425 port -- Stage E17: FIRST closed-loop BEMF-driven
 ...
 * instrumentation added there (stage_e17_first_closed_loop_snapshot
 * for ZC-state fields, stage_e17_first_timeout_diag for the first
 ...
 *
 * (1) stage_e17_regs_after_commutation: captured immediately after
 ...
 *
 * (2) stage_e17_regs_after_first_dma_irq: the SAME register set,
 ...
 *
 * stage_e17_first_timeout_diag (retained from Stage E16B) still
 ...
 *     points: stage_e17_tmr1_regs_after_commutation (immediately
 *     after the first closed-loop apply_step()) and
 *     stage_e17_tmr1_regs_at_timeout (at the first closed-loop
 *     timeout, right where stage_e17_first_timeout_diag is already
 ...
 *     stage_e17_tmr1_regs_after_commutation, and cval_1 (+ its TMR2
 ...
 *     "resume" step) -- stage_e17_last_arm_log (overwritten every
 *     call) plus separately latched stage_e17_first_commutation_arm_
 *     log and stage_e17_first_timeout_arm_log (each captured once,
 ...
 *     gated on stage_e17_mode==MODE_CLOSED_LOOP (tmr3_purpose is only
 ...
 *
 * Pass/fail is read directly off stage_e17_first_commutation_arm_log
 * vs stage_e17_first_commutation_isr_entry: elapsed_us there should
 ...
 * stage_e17_first_timeout_arm_log/_isr_entry should show elapsed_us
 ...
 *   9. stage_e17_tmr3_purpose = purpose           -- set purpose
 ...
 *
 * (1) stage_e17_sector_log[64]: one entry per CLOSED-LOOP commutation
 ...
 *
 * (2) stage_e17_timeout_log[32]: one entry per zc_timeout_count event
 ...
 *     watchdog's own requested_delay_us, from stage_e17_last_arm_log
 ...
 * zc_timeout_count increments more than once per stage_e17_timeout_log
 * entry's surrounding sector (visible directly from timeout_occurred
 * plus how many stage_e17_timeout_log entries share the same
...
volatile tmr3_purpose_t stage_e17_tmr3_purpose;

volatile run_mode_t stage_e17_mode;
volatile uint32_t stage_e17_step_index;
volatile uint32_t stage_e17_step_count;
volatile uint32_t stage_e17_consecutive_correct; /* live, during MODE_HOLD, toward handover */
volatile uint32_t stage_e17_at_target_step_count;

volatile int stage_e17_floating_phase, stage_e17_positive_phase, stage_e17_negative_phase;
volatile uint16_t stage_e17_adc_a, stage_e17_adc_b, stage_e17_adc_c;
volatile uint16_t stage_e17_floating_adc, stage_e17_positive_adc, stage_e17_negative_adc;
volatile int32_t stage_e17_floating_diff;
volatile uint32_t stage_e17_step_start_us;
volatile uint32_t stage_e17_current_step_period_us;

/* --- diagnostics requested for this stage --- */
volatile int stage_e17_handover_success;
volatile uint32_t stage_e17_handover_step_index;
volatile uint32_t stage_e17_closed_loop_step_count;
volatile uint32_t stage_e17_zc_count;
volatile uint32_t stage_e17_zc_timeout_count;
volatile uint32_t stage_e17_polarity_error_count;
volatile uint32_t stage_e17_sector_period_us_min, stage_e17_sector_period_us_max, stage_e17_sector_period_us_sum;
volatile uint32_t stage_e17_scheduled_delay_us_min, stage_e17_scheduled_delay_us_max, stage_e17_scheduled_delay_us_sum;
volatile uint32_t stage_e17_final_step;

volatile uint32_t stage_e17_heartbeat;
volatile int stage_e17_running;
volatile int stage_e17_aligning;
volatile uint32_t stage_e17_dma_error;
volatile uint32_t stage_e17_dma_fdt_count;
volatile uint32_t stage_e17_adc_conversion_count;
volatile uint32_t stage_e17_tmr1_ch4_event_count;
...
} stage_e17_commutation_snapshot_t;

stage_e17_commutation_snapshot_t stage_e17_first_closed_loop_snapshot;
volatile int stage_e17_first_closed_loop_snapshot_valid;
...
} stage_e17_first_timeout_diag_t;

stage_e17_first_timeout_diag_t stage_e17_first_timeout_diag;
volatile int stage_e17_first_timeout_diag_valid;

/* Live, per-sector accumulators -- reset every apply_step(), read out
 * into stage_e17_first_timeout_diag the first time a timeout occurs. */
...
/* --- Stage E17: raw ADC/DMA/CH4 register snapshots --- */
...
typedef struct {
  ...
  uint32_t dma_fdt_count;
} stage_e17_reg_snapshot_t;

stage_e17_reg_snapshot_t stage_e17_regs_after_commutation;
volatile int stage_e17_regs_after_commutation_valid;

stage_e17_reg_snapshot_t stage_e17_regs_after_first_dma_irq;
volatile int stage_e17_regs_after_first_dma_irq_valid;

static void capture_reg_snapshot(stage_e17_reg_snapshot_t *s)
{
  ...
  s->ch4_event_count = stage_e17_tmr1_ch4_event_count;
  s->dma_fdt_count = stage_e17_dma_fdt_count;
}

/* --- Stage E17: TMR1-specific register dump (ctrl1.tmren, cval, pr,
...
} stage_e17_tmr1_regs_t;

stage_e17_tmr1_regs_t stage_e17_tmr1_regs_after_commutation;
volatile int stage_e17_tmr1_regs_after_commutation_valid;

stage_e17_tmr1_regs_t stage_e17_tmr1_regs_at_timeout;
volatile int stage_e17_tmr1_regs_at_timeout_valid;

static void capture_tmr1_regs(stage_e17_tmr1_regs_t *s)
{
  s->ctrl1 = TMR1->ctrl1;
  s->cval = TMR1->cval;
  s->pr = TMR1->pr;
 ...
 * same instant as stage_e17_tmr1_regs_after_commutation; cval_1
...
} stage_e17_cval_progression_t;

stage_e17_cval_progression_t stage_e17_cval_progression;

/* --- Stage E17: TMR3 one-shot re-arm sequence logging --- */
...
typedef struct {
  ...
  uint32_t ists_after_clear; /* Stage E17: right after the fix's required re-clear (step 7) */
  uint32_t ists_before_resume;
} stage_e17_arm_log_t;

stage_e17_arm_log_t stage_e17_last_arm_log; /* overwritten every tmr3_arm_oneshot() call */

stage_e17_arm_log_t stage_e17_first_commutation_arm_log;
volatile int stage_e17_first_commutation_arm_log_valid;
stage_e17_arm_log_t stage_e17_first_timeout_arm_log;
volatile int stage_e17_first_timeout_arm_log_valid;
...
typedef struct {
  ...
  uint32_t cval;
  uint32_t pr;
} stage_e17_isr_entry_log_t;

stage_e17_isr_entry_log_t stage_e17_last_isr_entry_log; /* overwritten every TMR3 IRQ entry */

stage_e17_isr_entry_log_t stage_e17_first_commutation_isr_entry;
volatile int stage_e17_first_commutation_isr_entry_valid;
stage_e17_isr_entry_log_t stage_e17_first_timeout_isr_entry;
volatile int stage_e17_first_timeout_isr_entry_valid;

/* --- Stage E17: closed-loop stability ring buffers --- */
...
} stage_e17_sector_entry_t;

stage_e17_sector_entry_t stage_e17_sector_log[SECTOR_LOG_SIZE];
volatile uint32_t stage_e17_sector_log_count;
...
typedef struct {
  ...
  int zc_state;
  int32_t diff_min, diff_max;
} stage_e17_timeout_entry_t;

stage_e17_timeout_entry_t stage_e17_timeout_log[TIMEOUT_LOG_SIZE];
volatile uint32_t stage_e17_timeout_log_count;
...
void TMR1_CH_IRQHandler(void)
{
  if (tmr_flag_get(TMR1, TMR_C4_FLAG) != RESET) {
    ...
    stage_e17_tmr1_ch4_event_count++;
  }
}
...
static void apply_step(int idx, uint16_t duty_ccr)
{
  ...
  stage_e17_step_index = (uint32_t)idx;
  stage_e17_floating_phase = (int)s->floating;
  stage_e17_positive_phase = (int)s->pos;
  stage_e17_negative_phase = (int)s->neg;

  zc_arm_state = ZC_UNARMED;
  zc_locked = 0;
  blank_scans_remaining = COMMUTATION_BLANK_SCANS;
  stage_e17_step_start_us = TMR2->cval;

  /* Stage E17 instrumentation: fresh per-sector accumulators. */
  ...
  live_sector_dma_fdt_start = stage_e17_dma_fdt_count;
  live_sector_ch4_event_start = stage_e17_tmr1_ch4_event_count;
...
static void stop_and_force_off(void)
{
  ...
  gate_pins_force_off();
  stage_e17_running = 0;
  stage_e17_mode = MODE_STOPPED;
  stage_e17_final_step = stage_e17_step_index;
}

/*
 * Stage E17 FIX: Stage E16E's real hardware log confirmed
...
static void tmr3_arm_oneshot(uint32_t us, tmr3_purpose_t purpose)
{
  ...
  stage_e17_tmr3_purpose = purpose;                   /* 9. set purpose */
  ...
  stage_e17_current_step_period_us = ticks * TMR3_TICK_US;

  stage_e17_last_arm_log.arm_timestamp_us = arm_timestamp_us;
  stage_e17_last_arm_log.purpose = (int)purpose;
  stage_e17_last_arm_log.requested_delay_us = requested_delay_us;
  stage_e17_last_arm_log.ists_before_sw_trigger = ists_before_sw_trigger;
  stage_e17_last_arm_log.ists_after_sw_trigger = ists_after_sw_trigger;
  stage_e17_last_arm_log.ists_after_clear = ists_after_clear;
  stage_e17_last_arm_log.ists_before_resume = ists_before_resume;

  if (purpose == TMR3_PURPOSE_COMMUTATION && !stage_e17_first_commutation_arm_log_valid) {
    stage_e17_first_commutation_arm_log = stage_e17_last_arm_log;
    stage_e17_first_commutation_arm_log_valid = 1;
  }
  if (purpose == TMR3_PURPOSE_TIMEOUT && !stage_e17_first_timeout_arm_log_valid) {
    stage_e17_first_timeout_arm_log = stage_e17_last_arm_log;
    stage_e17_first_timeout_arm_log_valid = 1;
  }
}
...
static void start_ramp_bucket(uint32_t idx)
{
  ...
  stage_e17_current_step_period_us = period_ticks * TMR3_TICK_US;
  bucket_step_count = 0;
...
{
  stage_e17_mode = MODE_HOLD;
  stage_e17_current_step_period_us = HOLD_PERIOD_TICKS * TMR3_TICK_US;
  stage_e17_at_target_step_count = 0;
  stage_e17_consecutive_correct = 0;
...
{
  /*
   * Stage E17 instrumentation: logged BEFORE the flag check/early
   ...
   */
  {
    ...
    tmr3_purpose_t purpose_now = stage_e17_tmr3_purpose;
    uint32_t elapsed = (uint32_t)(uint16_t)(entry_ts - stage_e17_last_arm_log.arm_timestamp_us);

    stage_e17_last_isr_entry_log.isr_entry_timestamp_us = entry_ts;
    stage_e17_last_isr_entry_log.purpose = (int)purpose_now;
    stage_e17_last_isr_entry_log.elapsed_us = elapsed;
    stage_e17_last_isr_entry_log.ists = ists_now;
    stage_e17_last_isr_entry_log.cval = cval_now;
    stage_e17_last_isr_entry_log.pr = pr_now;
    ...
    if (stage_e17_mode == MODE_CLOSED_LOOP) {
      if (purpose_now == TMR3_PURPOSE_COMMUTATION && !stage_e17_first_commutation_isr_entry_valid) {
        stage_e17_first_commutation_isr_entry = stage_e17_last_isr_entry_log;
        stage_e17_first_commutation_isr_entry_valid = 1;
      }
      if (purpose_now == TMR3_PURPOSE_TIMEOUT && !stage_e17_first_timeout_isr_entry_valid) {
        stage_e17_first_timeout_isr_entry = stage_e17_last_isr_entry_log;
        stage_e17_first_timeout_isr_entry_valid = 1;
      }
    }
  }
  ...
  if (!stage_e17_running) return;

  if (stage_e17_mode == MODE_RAMP) {
    int next = (int)((stage_e17_step_index + 1u) % 6u);
    apply_step(next, g_duty_ccr);
    stage_e17_step_count++;
    bucket_step_count++;
  ...
  if (stage_e17_mode == MODE_HOLD) {
    int next = (int)((stage_e17_step_index + 1u) % 6u);
    apply_step(next, g_duty_ccr);
    stage_e17_step_count++;
    stage_e17_at_target_step_count++;

    if (stage_e17_at_target_step_count >= MAX_STEPS_AT_TARGET) {
  ...
  if (stage_e17_mode == MODE_CLOSED_LOOP) {
    if (stage_e17_tmr3_purpose == TMR3_PURPOSE_TIMEOUT) {
      /*
       ...
       * stops the motor.
       */
      stage_e17_zc_timeout_count++;
      live_timeout_count_this_sector++;

      /* Stage E17: append to the timeout ring buffer (capped, not
       * wrapping -- stops recording detail past TIMEOUT_LOG_SIZE
       * entries, zc_timeout_count itself keeps counting normally). */
      if (stage_e17_timeout_log_count < TIMEOUT_LOG_SIZE) {
        stage_e17_timeout_entry_t *te = &stage_e17_timeout_log[stage_e17_timeout_log_count];
        uint32_t now_us = TMR2->cval;
        te->current_step = stage_e17_step_index;
        te->time_since_commutation_us = (uint32_t)(uint16_t)(now_us - stage_e17_step_start_us);
        te->expected_timeout_us = stage_e17_last_arm_log.requested_delay_us;
        ...
        stage_e17_timeout_log_count++;
      }

      /* Stage E17 instrumentation: capture the FIRST closed-loop
       * timeout's full sector picture, before anything below re-arms
       * or resets it. */
      if (!stage_e17_first_timeout_diag_valid) {
        stage_e17_first_timeout_diag_t *d = &stage_e17_first_timeout_diag;
        ...
        d->last_adc_a = stage_e17_adc_a;
        d->last_adc_b = stage_e17_adc_b;
        d->last_adc_c = stage_e17_adc_c;
        d->last_floating_adc = stage_e17_floating_adc;
        d->last_positive_adc = stage_e17_positive_adc;
        d->last_negative_adc = stage_e17_negative_adc;
        d->dma_irq_count_delta = stage_e17_dma_fdt_count - live_sector_dma_fdt_start;
        d->ch4_event_count_delta = stage_e17_tmr1_ch4_event_count - live_sector_ch4_event_start;
        d->zc_state_at_timeout = (int)zc_arm_state;
        stage_e17_first_timeout_diag_valid = 1;

        capture_tmr1_regs(&stage_e17_tmr1_regs_at_timeout);
        stage_e17_tmr1_regs_at_timeout_valid = 1;
      }

      consecutive_fault_count++;
      check_fault_stop();
      if (stage_e17_running) arm_timeout_watchdog();
      return;
    }

    /* TMR3_PURPOSE_COMMUTATION: this overflow IS the ZC-scheduled
     * commutation. */
    uint32_t prev_step_start = stage_e17_step_start_us;
    uint32_t prev_step_index = stage_e17_step_index; /* the sector that is ENDING -- apply_step() below overwrites stage_e17_step_index */
    int next = (int)((stage_e17_step_index + 1u) % 6u);

    apply_step(next, g_duty_ccr);
    stage_e17_step_count++;
    stage_e17_closed_loop_step_count++;

    /* Stage E17 instrumentation: capture the FIRST closed-loop
     * commutation's state, immediately after apply_step() -- proof of
     * what apply_step() actually reset for the new sector. */
    if (stage_e17_closed_loop_step_count == 1u && !stage_e17_first_closed_loop_snapshot_valid) {
      stage_e17_commutation_snapshot_t *snap = &stage_e17_first_closed_loop_snapshot;
      snap->step_index = stage_e17_step_index;
      snap->expected_dir = expected_dir[stage_e17_step_index];
      snap->pos_phase = stage_e17_positive_phase;
      snap->neg_phase = stage_e17_negative_phase;
      snap->floating_phase = stage_e17_floating_phase;
      ...
      snap->dma_fdt_count = stage_e17_dma_fdt_count;
      stage_e17_first_closed_loop_snapshot_valid = 1;

      capture_reg_snapshot(&stage_e17_regs_after_commutation);
      stage_e17_regs_after_commutation_valid = 1;

      capture_tmr1_regs(&stage_e17_tmr1_regs_after_commutation);
      stage_e17_tmr1_regs_after_commutation_valid = 1;

      stage_e17_cval_progression.cval_0 = TMR1->cval;
      stage_e17_cval_progression.cval_0_timestamp_us = TMR2->cval;
    }

    uint32_t now_us = stage_e17_step_start_us; /* just set by apply_step() */
    uint32_t period_us = (uint32_t)(uint16_t)(now_us - prev_step_start);
    if (period_us < stage_e17_sector_period_us_min) stage_e17_sector_period_us_min = period_us;
    if (period_us > stage_e17_sector_period_us_max) stage_e17_sector_period_us_max = period_us;
    stage_e17_sector_period_us_sum += period_us;
    last_valid_sector_period_us = period_us;

    /* Stage E17: complete the sector-log entry for the sector that
     * just ended, using the pending_* values the DMA IRQ stashed at
     * the confirming ZC (before this commutation was even armed). */
    if (stage_e17_sector_log_count < SECTOR_LOG_SIZE) {
      stage_e17_sector_entry_t *se = &stage_e17_sector_log[stage_e17_sector_log_count];
      ...
      se->scheduled_delay_us = stage_e17_last_arm_log.requested_delay_us;
...
void ADC1_IRQHandler(void)
{
  if (adc_interrupt_flag_get(ADC1, ADC_CCE_FLAG) != RESET) {
    ...
    stage_e17_adc_conversion_count++;
  }
}
...
void DMA1_Channel1_IRQHandler(void)
{
  if (dma_interrupt_flag_get(DMA1_FDT1_FLAG) != RESET) {
    ...
    stage_e17_dma_fdt_count++;

    /* Stage E17 instrumentation: the first FDT IRQ after entering
     * closed loop -- does the hardware auto-reload/re-arm look healthy
     * right after this one successful transfer? */
    if (stage_e17_mode == MODE_CLOSED_LOOP &&
        stage_e17_regs_after_commutation_valid &&
        !stage_e17_regs_after_first_dma_irq_valid) {
      capture_reg_snapshot(&stage_e17_regs_after_first_dma_irq);
      stage_e17_regs_after_first_dma_irq_valid = 1;

      /* Stage E17: cval_1, the second point of the non-blocking
       * progression check -- same IRQ, no busy-wait added. */
      stage_e17_cval_progression.cval_1 = TMR1->cval;
      stage_e17_cval_progression.cval_1_timestamp_us = TMR2->cval;
      stage_e17_cval_progression.valid = 1;
    }
    ...
    if (stage_e17_running && !stage_e17_aligning) {
      uint16_t v = vals[stage_e17_floating_phase];
      uint16_t pos_v = vals[stage_e17_positive_phase];
      uint16_t neg_v = vals[stage_e17_negative_phase];

      stage_e17_floating_adc = v;
      stage_e17_positive_adc = pos_v;
      stage_e17_negative_adc = neg_v;

      int32_t diff = (int32_t)v - (int32_t)pos_v / 2;
      stage_e17_floating_diff = diff;
      ...
      if (blank_scans_remaining) {
        ...
        /* Stage E17 instrumentation: live per-sector accumulators. */
        ...
        if (confirmed_dir) {
          ...
          uint32_t now_us = TMR2->cval;
          uint32_t delay_us = (uint32_t)(uint16_t)(now_us - stage_e17_step_start_us);
          int matched = (confirmed_dir == expected_dir[stage_e17_step_index]);

          if (stage_e17_mode == MODE_RAMP || stage_e17_mode == MODE_HOLD) {
            if (matched) {
              stage_e17_consecutive_correct++;
            } else {
              stage_e17_consecutive_correct = 0;
            }

            if (stage_e17_mode == MODE_HOLD &&
                stage_e17_consecutive_correct >= HANDOVER_CONSECUTIVE_REQUIRED) {
              ...
              stage_e17_mode = MODE_CLOSED_LOOP;
              stage_e17_handover_success = 1;
              stage_e17_handover_step_index = stage_e17_step_index;

              /* Stage E17: stash this (handover) sector's log values
              ...
              stage_e17_sector_period_us_min = 0xffffffffu;
              stage_e17_sector_period_us_max = 0;
              stage_e17_sector_period_us_sum = 0;
              stage_e17_scheduled_delay_us_min = 0xffffffffu;
              stage_e17_scheduled_delay_us_max = 0;
              stage_e17_scheduled_delay_us_sum = 0;

              schedule_next_commutation(delay_us);
              stage_e17_scheduled_delay_us_min = delay_us;
              stage_e17_scheduled_delay_us_max = delay_us;
              stage_e17_scheduled_delay_us_sum = delay_us;
            }
          } else if (stage_e17_mode == MODE_CLOSED_LOOP) {
            stage_e17_zc_count++;

            if (matched) {
              /* Stage E17: stash this sector's log values for the
              ...
              if (delay_us < stage_e17_scheduled_delay_us_min) stage_e17_scheduled_delay_us_min = delay_us;
              if (delay_us > stage_e17_scheduled_delay_us_max) stage_e17_scheduled_delay_us_max = delay_us;
              stage_e17_scheduled_delay_us_sum += delay_us;
              ...
              stage_e17_polarity_error_count++;
              consecutive_fault_count++;
              check_fault_stop();
...
int main(void)
{
  ...
  tim1_pins_to_af();
  stage_e17_aligning = 1;
  stage_e17_running = 1;
  stage_e17_mode = MODE_RAMP;
  stage_e17_step_index = 0;
  apply_step(0, g_duty_ccr);
  tmr_output_enable(TMR1, TRUE);
  tmr_counter_enable(TMR1, TRUE);
  delay_us(ALIGN_DURATION_MS * 1000u);
  stage_e17_aligning = 0;

  ramp_bucket_index = 0;
  start_ramp_bucket(0);
  stage_e17_step_count = 1;
