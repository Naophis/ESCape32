/*
 * ESCape32 AT32F425 port -- Stage E9: PWM output vs. ADC-trigger-
 * position ground truth. Builds directly on Stage E8 (same frozen-
 * sector, same offset sweep, same ADC/DMA plumbing) -- NOT changed
 * per instruction. Adds exactly the six diagnostics the user asked
 * for to distinguish:
 ...
 *     never change across the sweep): stage_e9_tmr1_c1dt (CCR1),
 *     stage_e9_tmr1_cm1 (CH1/CH2 mode raw), stage_e9_tmr1_cctrl
 *     (CH1 enable/polarity raw, same register CH2-4 also live in),
 *     stage_e9_tmr1_brk (dead-time/MOE), stage_e9_tmr1_ctrl1 (CEN/
 *     direction).
 *
...
} stage_e9_offset_result_t;

stage_e9_offset_result_t stage_e9_results[NUM_OFFSETS];
volatile uint32_t stage_e9_offset_index;

volatile uint32_t stage_e9_heartbeat;
volatile int stage_e9_running;
volatile uint32_t stage_e9_sample_index;
volatile uint16_t stage_e9_floating_adc, stage_e9_positive_adc, stage_e9_negative_adc;
volatile uint32_t stage_e9_tmr1_ch4_event_count;
volatile uint32_t stage_e9_adc_conversion_count;
volatile uint32_t stage_e9_dma_fdt_count;
volatile uint32_t stage_e9_dma_error;
volatile uint32_t stage_e9_tim1_c4dt;

/* CH1 register dump, captured once (fixed sector). */
volatile uint32_t stage_e9_tmr1_c1dt;
volatile uint32_t stage_e9_tmr1_cm1;
volatile uint32_t stage_e9_tmr1_cctrl;
volatile uint32_t stage_e9_tmr1_brk;
volatile uint32_t stage_e9_tmr1_ctrl1;
...
static void tim1_adc_trigger_config(uint16_t first_offset_ticks)
{
  ...
  stage_e9_tim1_c4dt = first_offset_ticks;
}

static void update_adc_trigger_offset(uint16_t ticks)
{
  TMR1->c4dt = ticks;
  stage_e9_tim1_c4dt = ticks;
}
...
static void apply_fixed_step(uint16_t duty_ccr)
{
  ...
  stage_e9_tmr1_c1dt = TMR1->c1dt;
  stage_e9_tmr1_cm1 = TMR1->cm1;
  stage_e9_tmr1_cctrl = TMR1->cctrl;
  stage_e9_tmr1_brk = TMR1->brk;
  stage_e9_tmr1_ctrl1 = TMR1->ctrl1;
}
...
static void snapshot_offset(uint32_t idx)
{
  ...
  r->dma_error = stage_e9_dma_error;
...
void DMA1_Channel1_IRQHandler(void)
{
  if (dma_interrupt_flag_get(DMA1_FDT1_FLAG) != RESET) {
    ...
    stage_e9_floating_adc = floating_v;
    stage_e9_positive_adc = positive_v;
    stage_e9_negative_adc = negative_v;
    ...
    if (stage_e9_running) {
      ...
      stage_e9_sample_index++;
      if (stage_e9_sample_index >= SAMPLES_PER_OFFSET) {
        snapshot_offset(stage_e9_offset_index);
        stage_e9_offset_index++;
        if (stage_e9_offset_index >= NUM_OFFSETS) {
          stop_and_force_off();
        } else {
          start_offset(stage_e9_offset_index);
        }
      }
    }
  }
