/*
 * ESCape32 AT32F425 port -- Stage E16B: FIRST closed-loop BEMF-driven
 ...
 *
 * STAGE E16B ADDITION (nothing above this line changed -- handover
 * condition, TMR3 purpose split, 3089us-class scheduled delay, 15%
 * duty, ADC trigger config are all untouched): E16's first real run
 * handed over successfully at step1 (handover_step_index=1,
 * scheduled_delay_us=3089) and applied exactly one closed-loop
 * commutation (closed_loop_step_count=1, landing on step2), but then
 * got zero confirmed ZC (zc_count=0) through three full timeout
 * windows (zc_timeout_count=3) and stopped at final_step=2.
 *
 * Code review performed first, per instruction, before adding any new
 * instrumentation: apply_step() resets, IN THIS ORDER, for every
 * sector regardless of mode -- stage_e16b_step_index (updated to the
 * NEW step first), then floating/positive/negative_phase (derived
 * from steps[stage_e16b_step_index], i.e. computed AFTER the index
 * update, so role/expected_dir lookups by the new step_index are
 * correct from the first sample of the new sector) -- then
 * zc_arm_state=ZC_UNARMED, zc_locked=0, blank_scans_remaining=
 * COMMUTATION_BLANK_SCANS, step_start_us=TMR2->cval. This is the same
 * order and the same fields Stage E15's apply_step() reset, and
 * matches this file's own requirement list exactly (sector_decided/
 * zc_locked, zc_state/zc_arm_state, blanking count, and the step-index-
 * before-role-lookup ordering) -- no reset omission or ordering bug
 * was found by inspection. zc_active's own confirmed_sign/confirm_run
 * fields are NOT reset in apply_step() (same as Stage E15) -- they are
 * only ever consulted after an ARMED transition, which always
 * re-initializes both fields fresh before use, so a stale leftover
 * value between sectors is inert. This file does not change any of
 * that -- it only adds read-only instrumentation to observe it
 * directly instead of re-deriving it from source again:
 *
 * (1) stage_e16b_first_closed_loop_snapshot: captured once, immediately
 *     after the FIRST closed-loop apply_step() runs (closed_loop_step_
 *     count becomes 1) -- step_index, expected_dir[step], pos/neg/
 *     floating phase, sector_decided (zc_locked), zc_state
 *     (zc_arm_state), zc_active's confirmed_sign/confirm_run, the
 *     blanking counter, and the DMA IRQ (FDT) count at that instant --
 *     proof of what apply_step() actually left behind for the new
 *     sector to build on.
 *
 * (2) stage_e16b_first_timeout_diag: captured once, at the FIRST
 *     closed-loop timeout (zc_timeout_count becomes 1) -- ADC sample
 *     count processed during that sector (post-blanking), diff min/
 *     max observed, whether the sector ever armed at all, rising/
 *     falling confirm counts, the LAST raw ADC A/B/C and role-tagged
 *     floating/positive/negative values seen, the DMA IRQ count DELTA
 *     across the sector, and the zc_state at the moment of timeout.
 *     This is the direct answer to "why did step2 never confirm": if
 *     zc_state_at_timeout is still UNARMED with diff_min/max both
 *     inside the deadband, diff never moved far enough at all (no
 *     usable BEMF signal reached the ADC that sector); if it DID arm
 *     but never confirmed, the ADC/DMA/CH4 chain and the arm/confirm
 *     mechanism are both alive (armed_count=1, dma count delta
 *     nonzero) but the crossing back over the opposite threshold
 *     never completed -- two very different failure modes this data
 *     distinguishes directly instead of guessing.
 */
...
volatile tmr3_purpose_t stage_e16b_tmr3_purpose;

volatile run_mode_t stage_e16b_mode;
volatile uint32_t stage_e16b_step_index;
volatile uint32_t stage_e16b_step_count;
volatile uint32_t stage_e16b_consecutive_correct; /* live, during MODE_HOLD, toward handover */
volatile uint32_t stage_e16b_at_target_step_count;

volatile int stage_e16b_floating_phase, stage_e16b_positive_phase, stage_e16b_negative_phase;
volatile uint16_t stage_e16b_adc_a, stage_e16b_adc_b, stage_e16b_adc_c;
volatile uint16_t stage_e16b_floating_adc, stage_e16b_positive_adc, stage_e16b_negative_adc;
volatile int32_t stage_e16b_floating_diff;
volatile uint32_t stage_e16b_step_start_us;
volatile uint32_t stage_e16b_current_step_period_us;

/* --- diagnostics requested for this stage --- */
volatile int stage_e16b_handover_success;
volatile uint32_t stage_e16b_handover_step_index;
volatile uint32_t stage_e16b_closed_loop_step_count;
volatile uint32_t stage_e16b_zc_count;
volatile uint32_t stage_e16b_zc_timeout_count;
volatile uint32_t stage_e16b_polarity_error_count;
volatile uint32_t stage_e16b_sector_period_us_min, stage_e16b_sector_period_us_max, stage_e16b_sector_period_us_sum;
volatile uint32_t stage_e16b_scheduled_delay_us_min, stage_e16b_scheduled_delay_us_max, stage_e16b_scheduled_delay_us_sum;
volatile uint32_t stage_e16b_final_step;

volatile uint32_t stage_e16b_heartbeat;
volatile int stage_e16b_running;
volatile int stage_e16b_aligning;
volatile uint32_t stage_e16b_dma_error;
volatile uint32_t stage_e16b_dma_fdt_count;
volatile uint32_t stage_e16b_adc_conversion_count;
volatile uint32_t stage_e16b_tmr1_ch4_event_count;
...
} stage_e16b_commutation_snapshot_t;

stage_e16b_commutation_snapshot_t stage_e16b_first_closed_loop_snapshot;
volatile int stage_e16b_first_closed_loop_snapshot_valid;
...
} stage_e16b_first_timeout_diag_t;

stage_e16b_first_timeout_diag_t stage_e16b_first_timeout_diag;
volatile int stage_e16b_first_timeout_diag_valid;

/* Live, per-sector accumulators -- reset every apply_step(), read out
 * into stage_e16b_first_timeout_diag the first time a timeout occurs. */
...
void TMR1_CH_IRQHandler(void)
{
  if (tmr_flag_get(TMR1, TMR_C4_FLAG) != RESET) {
    ...
    stage_e16b_tmr1_ch4_event_count++;
  }
}
...
static void apply_step(int idx, uint16_t duty_ccr)
{
  ...
  stage_e16b_step_index = (uint32_t)idx;
  stage_e16b_floating_phase = (int)s->floating;
  stage_e16b_positive_phase = (int)s->pos;
  stage_e16b_negative_phase = (int)s->neg;

  zc_arm_state = ZC_UNARMED;
  zc_locked = 0;
  blank_scans_remaining = COMMUTATION_BLANK_SCANS;
  stage_e16b_step_start_us = TMR2->cval;

  /* Stage E16B instrumentation: fresh per-sector accumulators. */
  ...
  live_sector_dma_fdt_start = stage_e16b_dma_fdt_count;
}
...
static void stop_and_force_off(void)
{
  ...
  gate_pins_force_off();
  stage_e16b_running = 0;
  stage_e16b_mode = MODE_STOPPED;
  stage_e16b_final_step = stage_e16b_step_index;
}
...
static void tmr3_arm_oneshot(uint32_t us)
{
  ...
  stage_e16b_current_step_period_us = ticks * TMR3_TICK_US;
}
...
static void schedule_next_commutation(uint32_t delay_us)
{
  ...
  stage_e16b_tmr3_purpose = TMR3_PURPOSE_COMMUTATION;
...
static void arm_timeout_watchdog(void)
{
  ...
  stage_e16b_tmr3_purpose = TMR3_PURPOSE_TIMEOUT;
}
...
static void start_ramp_bucket(uint32_t idx)
{
  ...
  stage_e16b_current_step_period_us = period_ticks * TMR3_TICK_US;
  bucket_step_count = 0;
...
{
  stage_e16b_mode = MODE_HOLD;
  stage_e16b_current_step_period_us = HOLD_PERIOD_TICKS * TMR3_TICK_US;
  stage_e16b_at_target_step_count = 0;
  stage_e16b_consecutive_correct = 0;
...
void TMR3_GLOBAL_IRQHandler(void)
{
  ...
  if (!stage_e16b_running) return;

  if (stage_e16b_mode == MODE_RAMP) {
    int next = (int)((stage_e16b_step_index + 1u) % 6u);
    apply_step(next, g_duty_ccr);
    stage_e16b_step_count++;
    bucket_step_count++;
  ...
  if (stage_e16b_mode == MODE_HOLD) {
    int next = (int)((stage_e16b_step_index + 1u) % 6u);
    apply_step(next, g_duty_ccr);
    stage_e16b_step_count++;
    stage_e16b_at_target_step_count++;

    if (stage_e16b_at_target_step_count >= MAX_STEPS_AT_TARGET) {
  ...
  if (stage_e16b_mode == MODE_CLOSED_LOOP) {
    if (stage_e16b_tmr3_purpose == TMR3_PURPOSE_TIMEOUT) {
      /*
       ...
       * stops the motor.
       */
      stage_e16b_zc_timeout_count++;

      /* Stage E16B instrumentation: capture the FIRST closed-loop
       * timeout's full sector picture, before anything below re-arms
       * or resets it. */
      if (!stage_e16b_first_timeout_diag_valid) {
        stage_e16b_first_timeout_diag_t *d = &stage_e16b_first_timeout_diag;
        ...
        d->last_adc_a = stage_e16b_adc_a;
        d->last_adc_b = stage_e16b_adc_b;
        d->last_adc_c = stage_e16b_adc_c;
        d->last_floating_adc = stage_e16b_floating_adc;
        d->last_positive_adc = stage_e16b_positive_adc;
        d->last_negative_adc = stage_e16b_negative_adc;
        d->dma_irq_count_delta = stage_e16b_dma_fdt_count - live_sector_dma_fdt_start;
        d->zc_state_at_timeout = (int)zc_arm_state;
        stage_e16b_first_timeout_diag_valid = 1;
      }

      consecutive_fault_count++;
      check_fault_stop();
      if (stage_e16b_running) arm_timeout_watchdog();
      return;
    }

    /* TMR3_PURPOSE_COMMUTATION: this overflow IS the ZC-scheduled
     * commutation. */
    uint32_t prev_step_start = stage_e16b_step_start_us;
    int next = (int)((stage_e16b_step_index + 1u) % 6u);

    apply_step(next, g_duty_ccr);
    stage_e16b_step_count++;
    stage_e16b_closed_loop_step_count++;

    /* Stage E16B instrumentation: capture the FIRST closed-loop
     * commutation's state, immediately after apply_step() -- proof of
     * what apply_step() actually reset for the new sector. */
    if (stage_e16b_closed_loop_step_count == 1u && !stage_e16b_first_closed_loop_snapshot_valid) {
      stage_e16b_commutation_snapshot_t *snap = &stage_e16b_first_closed_loop_snapshot;
      snap->step_index = stage_e16b_step_index;
      snap->expected_dir = expected_dir[stage_e16b_step_index];
      snap->pos_phase = stage_e16b_positive_phase;
      snap->neg_phase = stage_e16b_negative_phase;
      snap->floating_phase = stage_e16b_floating_phase;
      ...
      snap->dma_fdt_count = stage_e16b_dma_fdt_count;
      stage_e16b_first_closed_loop_snapshot_valid = 1;
    }

    uint32_t now_us = stage_e16b_step_start_us; /* just set by apply_step() */
    uint32_t period_us = (uint32_t)(uint16_t)(now_us - prev_step_start);
    if (period_us < stage_e16b_sector_period_us_min) stage_e16b_sector_period_us_min = period_us;
    if (period_us > stage_e16b_sector_period_us_max) stage_e16b_sector_period_us_max = period_us;
    stage_e16b_sector_period_us_sum += period_us;
...
void ADC1_IRQHandler(void)
{
  if (adc_interrupt_flag_get(ADC1, ADC_CCE_FLAG) != RESET) {
    ...
    stage_e16b_adc_conversion_count++;
  }
}
...
void DMA1_Channel1_IRQHandler(void)
{
  if (dma_interrupt_flag_get(DMA1_FDT1_FLAG) != RESET) {
    ...
    if (stage_e16b_running && !stage_e16b_aligning) {
      uint16_t v = vals[stage_e16b_floating_phase];
      uint16_t pos_v = vals[stage_e16b_positive_phase];
      uint16_t neg_v = vals[stage_e16b_negative_phase];

      stage_e16b_floating_adc = v;
      stage_e16b_positive_adc = pos_v;
      stage_e16b_negative_adc = neg_v;

      int32_t diff = (int32_t)v - (int32_t)pos_v / 2;
      stage_e16b_floating_diff = diff;
      ...
      if (blank_scans_remaining) {
        ...
        /* Stage E16B instrumentation: live per-sector accumulators. */
        ...
        if (confirmed_dir) {
          ...
          uint32_t delay_us = (uint32_t)(uint16_t)(now_us - stage_e16b_step_start_us);
          int matched = (confirmed_dir == expected_dir[stage_e16b_step_index]);

          if (stage_e16b_mode == MODE_RAMP || stage_e16b_mode == MODE_HOLD) {
            if (matched) {
              stage_e16b_consecutive_correct++;
            } else {
              stage_e16b_consecutive_correct = 0;
            }

            if (stage_e16b_mode == MODE_HOLD &&
                stage_e16b_consecutive_correct >= HANDOVER_CONSECUTIVE_REQUIRED) {
              ...
              stage_e16b_mode = MODE_CLOSED_LOOP;
              stage_e16b_handover_success = 1;
              stage_e16b_handover_step_index = stage_e16b_step_index;
              consecutive_fault_count = 0;

              stage_e16b_sector_period_us_min = 0xffffffffu;
              stage_e16b_sector_period_us_max = 0;
              stage_e16b_sector_period_us_sum = 0;
              stage_e16b_scheduled_delay_us_min = 0xffffffffu;
              stage_e16b_scheduled_delay_us_max = 0;
              stage_e16b_scheduled_delay_us_sum = 0;

              schedule_next_commutation(delay_us);
              stage_e16b_scheduled_delay_us_min = delay_us;
              stage_e16b_scheduled_delay_us_max = delay_us;
              stage_e16b_scheduled_delay_us_sum = delay_us;
            }
          } else if (stage_e16b_mode == MODE_CLOSED_LOOP) {
            stage_e16b_zc_count++;

            if (matched) {
              consecutive_fault_count = 0;

              if (delay_us < stage_e16b_scheduled_delay_us_min) stage_e16b_scheduled_delay_us_min = delay_us;
              if (delay_us > stage_e16b_scheduled_delay_us_max) stage_e16b_scheduled_delay_us_max = delay_us;
              stage_e16b_scheduled_delay_us_sum += delay_us;
              ...
              stage_e16b_polarity_error_count++;
              consecutive_fault_count++;
              check_fault_stop();
...
int main(void)
{
  ...
  tim1_pins_to_af();
  stage_e16b_aligning = 1;
  stage_e16b_running = 1;
  stage_e16b_mode = MODE_RAMP;
  stage_e16b_step_index = 0;
  apply_step(0, g_duty_ccr);
  tmr_output_enable(TMR1, TRUE);
  tmr_counter_enable(TMR1, TRUE);
  delay_us(ALIGN_DURATION_MS * 1000u);
  stage_e16b_aligning = 0;

  ramp_bucket_index = 0;
  start_ramp_bucket(0);
  stage_e16b_step_count = 1;
  step_timer_init(ramp_period_ticks[0]);
  tmr_counter_enable(TMR3, TRUE);

  for (;;) {
    ++stage_e16b_heartbeat;
  }
}
