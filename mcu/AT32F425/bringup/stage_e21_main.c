/*
 * ESCape32 AT32F425 port -- Stage E21: FIRST closed-loop BEMF-driven
 ...
 * instrumentation added there (stage_e21_first_closed_loop_snapshot
 * for ZC-state fields, stage_e21_first_timeout_diag for the first
 ...
 *
 * (1) stage_e21_regs_after_commutation: captured immediately after
 ...
 *
 * (2) stage_e21_regs_after_first_dma_irq: the SAME register set,
 ...
 *
 * stage_e21_first_timeout_diag (retained from Stage E16B) still
 ...
 *     points: stage_e21_tmr1_regs_after_commutation (immediately
 *     after the first closed-loop apply_step()) and
 *     stage_e21_tmr1_regs_at_timeout (at the first closed-loop
 *     timeout, right where stage_e21_first_timeout_diag is already
 ...
 *     stage_e21_tmr1_regs_after_commutation, and cval_1 (+ its TMR2
 ...
 *     "resume" step) -- stage_e21_last_arm_log (overwritten every
 *     call) plus separately latched stage_e21_first_commutation_arm_
 *     log and stage_e21_first_timeout_arm_log (each captured once,
 ...
 *     gated on stage_e21_mode==MODE_CLOSED_LOOP (tmr3_purpose is only
 ...
 *
 * Pass/fail is read directly off stage_e21_first_commutation_arm_log
 * vs stage_e21_first_commutation_isr_entry: elapsed_us there should
 ...
 * stage_e21_first_timeout_arm_log/_isr_entry should show elapsed_us
 ...
 *   9. stage_e21_tmr3_purpose = purpose           -- set purpose
 ...
 *
 * (1) stage_e21_sector_log[64]: one entry per CLOSED-LOOP commutation
 ...
 *
 * (2) stage_e21_timeout_log[32]: one entry per zc_timeout_count event
 ...
 *     watchdog's own requested_delay_us, from stage_e21_last_arm_log
 ...
 * zc_timeout_count increments more than once per stage_e21_timeout_log
 * entry's surrounding sector (visible directly from timeout_occurred
 * plus how many stage_e21_timeout_log entries share the same
 ...
 *
 * (1) stage_e21_sector_log[64] gains positive_adc_min/max/sum,
 ...
 *
 * (2) stage_e21_step_trace[3][16]: a short raw trace (positive_adc/
 ...
 * record (stage_e21_alllow_log[8]) to keep RAM bounded; EVERY event
 * still updates the per-step aggregates stage_e21_alllow_event_count[6]
 * and stage_e21_alllow_longest_duration_us[6] regardless of whether
 ...
 * stage_e21_stop_reason records which of NONE/FAULT/HANDOVER_TIMEOUT
 ...
 *       - stage_e21_sector_period_by_step[6]: min/max/sum/count of
 ...
 *       - stage_e21_zc_delay_by_step[6]: min/max/sum/count of the
 *         measured ZC-to-next-commutation delay (the 30-degree value),
 *         filed under the step whose ZC produced it.
 *       - stage_e21_sector_period_overall / stage_e21_zc_delay_overall:
 ...
 *         per-step tables.
 *       - stage_e21_alllow_ms_event_count: count of all-low events
...
/* Stage E21: why the run ended. */
typedef enum {
  STOP_REASON_NONE = 0,
  STOP_REASON_FAULT = 1,             /* consecutive_fault_count threshold (pre-existing) */
  STOP_REASON_HANDOVER_TIMEOUT = 2,  /* MAX_STEPS_AT_TARGET reached without handover (pre-existing) */
  STOP_REASON_MAX_COMMUTATIONS = 3,  /* new: 60 closed-loop commutations reached */
  STOP_REASON_MAX_TIME = 4           /* new: 500ms since handover reached */
} stop_reason_t;
volatile stop_reason_t stage_e21_stop_reason;

#define TEST_MAX_CLOSED_LOOP_COMMUTATIONS 60u
#define TEST_MAX_TIME_SINCE_HANDOVER_US 500000u

volatile uint32_t stage_e21_handover_timestamp_us;
...
volatile tmr3_purpose_t stage_e21_tmr3_purpose;

volatile run_mode_t stage_e21_mode;
volatile uint32_t stage_e21_step_index;
volatile uint32_t stage_e21_step_count;
volatile uint32_t stage_e21_consecutive_correct; /* live, during MODE_HOLD, toward handover */
volatile uint32_t stage_e21_at_target_step_count;

volatile int stage_e21_floating_phase, stage_e21_positive_phase, stage_e21_negative_phase;
volatile uint16_t stage_e21_adc_a, stage_e21_adc_b, stage_e21_adc_c;
volatile uint16_t stage_e21_floating_adc, stage_e21_positive_adc, stage_e21_negative_adc;
volatile int32_t stage_e21_floating_diff;
volatile uint32_t stage_e21_step_start_us;
volatile uint32_t stage_e21_current_step_period_us;

/* --- diagnostics requested for this stage --- */
volatile int stage_e21_handover_success;
volatile uint32_t stage_e21_handover_step_index;
volatile uint32_t stage_e21_closed_loop_step_count;
volatile uint32_t stage_e21_zc_count;
volatile uint32_t stage_e21_zc_timeout_count;
volatile uint32_t stage_e21_polarity_error_count;
volatile uint32_t stage_e21_sector_period_us_min, stage_e21_sector_period_us_max, stage_e21_sector_period_us_sum;
volatile uint32_t stage_e21_scheduled_delay_us_min, stage_e21_scheduled_delay_us_max, stage_e21_scheduled_delay_us_sum;
volatile uint32_t stage_e21_final_step;

/* --- Stage E21: per-step timing characterization --- */

typedef struct {
  uint32_t min, max, sum, count;
} stage_e21_stat_t;

stage_e21_stat_t stage_e21_sector_period_by_step[6];
stage_e21_stat_t stage_e21_zc_delay_by_step[6];
stage_e21_stat_t stage_e21_sector_period_overall; /* sum across all 6 steps -- mean = sum/count */
stage_e21_stat_t stage_e21_zc_delay_overall;

volatile uint32_t stage_e21_alllow_ms_event_count; /* all-low events (Stage E19 detector) with duration_us >= 1000 */

static void stat_update(stage_e21_stat_t *s, uint32_t value)
{
...
volatile uint32_t stage_e21_heartbeat;
volatile int stage_e21_running;
volatile int stage_e21_aligning;
volatile uint32_t stage_e21_dma_error;
volatile uint32_t stage_e21_dma_fdt_count;
volatile uint32_t stage_e21_adc_conversion_count;
volatile uint32_t stage_e21_tmr1_ch4_event_count;
...
} stage_e21_commutation_snapshot_t;

stage_e21_commutation_snapshot_t stage_e21_first_closed_loop_snapshot;
volatile int stage_e21_first_closed_loop_snapshot_valid;
...
} stage_e21_first_timeout_diag_t;

stage_e21_first_timeout_diag_t stage_e21_first_timeout_diag;
volatile int stage_e21_first_timeout_diag_valid;

/* Live, per-sector accumulators -- reset every apply_step(), read out
 * into stage_e21_first_timeout_diag the first time a timeout occurs. */
...
/* --- Stage E21: raw ADC/DMA/CH4 register snapshots --- */
...
typedef struct {
  ...
  uint32_t dma_fdt_count;
} stage_e21_reg_snapshot_t;

stage_e21_reg_snapshot_t stage_e21_regs_after_commutation;
volatile int stage_e21_regs_after_commutation_valid;

stage_e21_reg_snapshot_t stage_e21_regs_after_first_dma_irq;
volatile int stage_e21_regs_after_first_dma_irq_valid;

static void capture_reg_snapshot(stage_e21_reg_snapshot_t *s)
{
  ...
  s->ch4_event_count = stage_e21_tmr1_ch4_event_count;
  s->dma_fdt_count = stage_e21_dma_fdt_count;
}

/* --- Stage E21: TMR1-specific register dump (ctrl1.tmren, cval, pr,
...
} stage_e21_tmr1_regs_t;

stage_e21_tmr1_regs_t stage_e21_tmr1_regs_after_commutation;
volatile int stage_e21_tmr1_regs_after_commutation_valid;

stage_e21_tmr1_regs_t stage_e21_tmr1_regs_at_timeout;
volatile int stage_e21_tmr1_regs_at_timeout_valid;

static void capture_tmr1_regs(stage_e21_tmr1_regs_t *s)
{
  s->ctrl1 = TMR1->ctrl1;
  s->cval = TMR1->cval;
  s->pr = TMR1->pr;
 ...
 * same instant as stage_e21_tmr1_regs_after_commutation; cval_1
...
} stage_e21_cval_progression_t;

stage_e21_cval_progression_t stage_e21_cval_progression;

/* --- Stage E21: TMR3 one-shot re-arm sequence logging --- */
...
typedef struct {
  ...
  uint32_t ists_after_clear; /* Stage E21: right after the fix's required re-clear (step 7) */
  uint32_t ists_before_resume;
} stage_e21_arm_log_t;

stage_e21_arm_log_t stage_e21_last_arm_log; /* overwritten every tmr3_arm_oneshot() call */

stage_e21_arm_log_t stage_e21_first_commutation_arm_log;
volatile int stage_e21_first_commutation_arm_log_valid;
stage_e21_arm_log_t stage_e21_first_timeout_arm_log;
volatile int stage_e21_first_timeout_arm_log_valid;
...
typedef struct {
  ...
  uint32_t cval;
  uint32_t pr;
} stage_e21_isr_entry_log_t;

stage_e21_isr_entry_log_t stage_e21_last_isr_entry_log; /* overwritten every TMR3 IRQ entry */

stage_e21_isr_entry_log_t stage_e21_first_commutation_isr_entry;
volatile int stage_e21_first_commutation_isr_entry_valid;
stage_e21_isr_entry_log_t stage_e21_first_timeout_isr_entry;
volatile int stage_e21_first_timeout_isr_entry_valid;

/* --- Stage E21: closed-loop stability ring buffers --- */
...
typedef struct {
  ...
  /* --- Stage E21: per-sector ADC aggregation, post-blanking-to-ZC --- */
  ...
  int32_t diff_at_zc;
} stage_e21_sector_entry_t;

stage_e21_sector_entry_t stage_e21_sector_log[SECTOR_LOG_SIZE];
volatile uint32_t stage_e21_sector_log_count;
...
typedef struct {
  ...
  int zc_state;
  int32_t diff_min, diff_max;
} stage_e21_timeout_entry_t;

stage_e21_timeout_entry_t stage_e21_timeout_log[TIMEOUT_LOG_SIZE];
volatile uint32_t stage_e21_timeout_log_count;
...
static uint16_t pending_negative_adc_at_zc; /* Stage E21 */
static int32_t pending_diff_at_zc;           /* Stage E21 */
...
/* Live, per-sector ADC aggregation (Stage E21) -- reset in apply_step(),
...
/* --- Stage E21: short raw trace, first closed-loop occurrence of
...
} stage_e21_trace_sample_t;

stage_e21_trace_sample_t stage_e21_step_trace[STEP_TRACE_GROUPS][STEP_TRACE_LEN];
volatile uint32_t stage_e21_step_trace_fill_count[STEP_TRACE_GROUPS];
volatile int stage_e21_step_trace_captured[STEP_TRACE_GROUPS];
...
/* --- Stage E21: 3-phase all-low (suspected OCP/Hi-Z) event detector.
...
} stage_e21_alllow_event_t;

stage_e21_alllow_event_t stage_e21_alllow_log[ALLLOW_LOG_SIZE];
volatile uint32_t stage_e21_alllow_log_count;

volatile uint32_t stage_e21_alllow_event_count[6];
volatile uint32_t stage_e21_alllow_longest_duration_us[6];
...
static void alllow_finish_event(uint32_t end_us)
{
  ...
  stage_e21_alllow_event_count[step]++;
  if (duration_us > stage_e21_alllow_longest_duration_us[step]) {
    stage_e21_alllow_longest_duration_us[step] = duration_us;
  }
  /* Stage E21: distinguish genuine ms-scale events from the ~2us
   * break-before-make blip the detector now also sees every commutation. */
  if (duration_us >= 1000u) {
    stage_e21_alllow_ms_event_count++;
  }

  if (stage_e21_alllow_log_count < ALLLOW_LOG_SIZE) {
    stage_e21_alllow_event_t *ev = &stage_e21_alllow_log[stage_e21_alllow_log_count];
...
static void alllow_update(uint16_t a, uint16_t b, uint16_t c)
{
  ...
  if (low) {
    if (!alllow_active) {
      ...
      alllow_step_index = stage_e21_step_index;
      alllow_commutation_us = stage_e21_step_start_us;
      alllow_scan_count = 0;
...
void TMR1_CH_IRQHandler(void)
{
  if (tmr_flag_get(TMR1, TMR_C4_FLAG) != RESET) {
    ...
    stage_e21_tmr1_ch4_event_count++;
  }
}
...
static void apply_step(int idx, uint16_t duty_ccr)
{
  ...
  /*
   * Standard commutation sequence (Stage E21): old sector off -> break
  ...
  stage_e21_step_index = (uint32_t)idx;
  stage_e21_floating_phase = (int)s->floating;
  stage_e21_positive_phase = (int)s->pos;
  stage_e21_negative_phase = (int)s->neg;

  zc_arm_state = ZC_UNARMED;
  zc_locked = 0;
  blank_scans_remaining = COMMUTATION_BLANK_SCANS;
  stage_e21_step_start_us = TMR2->cval;

  /* Stage E21 instrumentation: fresh per-sector accumulators. */
  ...
  live_sector_dma_fdt_start = stage_e21_dma_fdt_count;
  live_sector_ch4_event_start = stage_e21_tmr1_ch4_event_count;
  live_timeout_count_this_sector = 0;

  /* Stage E21: fresh per-sector ADC aggregation. */
  ...
  /* Stage E21: start a trace only for the FIRST closed-loop occurrence
   * of step1/step3/step5, one-shot per group. */
  if (stage_e21_mode == MODE_CLOSED_LOOP) {
    int g = step_trace_group_for((uint32_t)idx);
    if (g >= 0 && !stage_e21_step_trace_captured[g]) {
      trace_active_group = g;
      stage_e21_step_trace_fill_count[g] = 0;
    } else {
...
{
  /* Stage E21: close out any all-low event still active so nothing is
   * left dangling. */
  if (alllow_active) {
    alllow_finish_event(TMR2->cval);
  }
  ...
  gate_pins_force_off();
  stage_e21_running = 0;
  stage_e21_mode = MODE_STOPPED;
  stage_e21_final_step = stage_e21_step_index;
  stage_e21_stop_reason = reason;
}

/*
 * Stage E21 FIX: Stage E16E's real hardware log confirmed
...
static void tmr3_arm_oneshot(uint32_t us, tmr3_purpose_t purpose)
{
  ...
  stage_e21_tmr3_purpose = purpose;                   /* 9. set purpose */
  ...
  stage_e21_current_step_period_us = ticks * TMR3_TICK_US;

  stage_e21_last_arm_log.arm_timestamp_us = arm_timestamp_us;
  stage_e21_last_arm_log.purpose = (int)purpose;
  stage_e21_last_arm_log.requested_delay_us = requested_delay_us;
  stage_e21_last_arm_log.ists_before_sw_trigger = ists_before_sw_trigger;
  stage_e21_last_arm_log.ists_after_sw_trigger = ists_after_sw_trigger;
  stage_e21_last_arm_log.ists_after_clear = ists_after_clear;
  stage_e21_last_arm_log.ists_before_resume = ists_before_resume;

  if (purpose == TMR3_PURPOSE_COMMUTATION && !stage_e21_first_commutation_arm_log_valid) {
    stage_e21_first_commutation_arm_log = stage_e21_last_arm_log;
    stage_e21_first_commutation_arm_log_valid = 1;
  }
  if (purpose == TMR3_PURPOSE_TIMEOUT && !stage_e21_first_timeout_arm_log_valid) {
    stage_e21_first_timeout_arm_log = stage_e21_last_arm_log;
    stage_e21_first_timeout_arm_log_valid = 1;
  }
}
...
static void start_ramp_bucket(uint32_t idx)
{
  ...
  stage_e21_current_step_period_us = period_ticks * TMR3_TICK_US;
  bucket_step_count = 0;
...
{
  stage_e21_mode = MODE_HOLD;
  stage_e21_current_step_period_us = HOLD_PERIOD_TICKS * TMR3_TICK_US;
  stage_e21_at_target_step_count = 0;
  stage_e21_consecutive_correct = 0;
...
{
  /*
   * Stage E21 instrumentation: logged BEFORE the flag check/early
   ...
   */
  {
    ...
    tmr3_purpose_t purpose_now = stage_e21_tmr3_purpose;
    uint32_t elapsed = (uint32_t)(uint16_t)(entry_ts - stage_e21_last_arm_log.arm_timestamp_us);

    stage_e21_last_isr_entry_log.isr_entry_timestamp_us = entry_ts;
    stage_e21_last_isr_entry_log.purpose = (int)purpose_now;
    stage_e21_last_isr_entry_log.elapsed_us = elapsed;
    stage_e21_last_isr_entry_log.ists = ists_now;
    stage_e21_last_isr_entry_log.cval = cval_now;
    stage_e21_last_isr_entry_log.pr = pr_now;
    ...
    if (stage_e21_mode == MODE_CLOSED_LOOP) {
      if (purpose_now == TMR3_PURPOSE_COMMUTATION && !stage_e21_first_commutation_isr_entry_valid) {
        stage_e21_first_commutation_isr_entry = stage_e21_last_isr_entry_log;
        stage_e21_first_commutation_isr_entry_valid = 1;
      }
      if (purpose_now == TMR3_PURPOSE_TIMEOUT && !stage_e21_first_timeout_isr_entry_valid) {
        stage_e21_first_timeout_isr_entry = stage_e21_last_isr_entry_log;
        stage_e21_first_timeout_isr_entry_valid = 1;
      }
    }
  }
  ...
  if (!stage_e21_running) return;

  if (stage_e21_mode == MODE_RAMP) {
    int next = (int)((stage_e21_step_index + 1u) % 6u);
    apply_step(next, g_duty_ccr);
    stage_e21_step_count++;
    bucket_step_count++;
  ...
  if (stage_e21_mode == MODE_HOLD) {
    int next = (int)((stage_e21_step_index + 1u) % 6u);
    apply_step(next, g_duty_ccr);
    stage_e21_step_count++;
    stage_e21_at_target_step_count++;

    if (stage_e21_at_target_step_count >= MAX_STEPS_AT_TARGET) {
  ...
  if (stage_e21_mode == MODE_CLOSED_LOOP) {
    if (stage_e21_tmr3_purpose == TMR3_PURPOSE_TIMEOUT) {
      /*
       ...
       * stops the motor.
       */
      stage_e21_zc_timeout_count++;
      live_timeout_count_this_sector++;

      /* Stage E21: append to the timeout ring buffer (capped, not
       * wrapping -- stops recording detail past TIMEOUT_LOG_SIZE
       * entries, zc_timeout_count itself keeps counting normally). */
      if (stage_e21_timeout_log_count < TIMEOUT_LOG_SIZE) {
        stage_e21_timeout_entry_t *te = &stage_e21_timeout_log[stage_e21_timeout_log_count];
        uint32_t now_us = TMR2->cval;
        te->current_step = stage_e21_step_index;
        te->time_since_commutation_us = (uint32_t)(uint16_t)(now_us - stage_e21_step_start_us);
        te->expected_timeout_us = stage_e21_last_arm_log.requested_delay_us;
        ...
        stage_e21_timeout_log_count++;
      }

      /* Stage E21 instrumentation: capture the FIRST closed-loop
       * timeout's full sector picture, before anything below re-arms
       * or resets it. */
      if (!stage_e21_first_timeout_diag_valid) {
        stage_e21_first_timeout_diag_t *d = &stage_e21_first_timeout_diag;
        ...
        d->last_adc_a = stage_e21_adc_a;
        d->last_adc_b = stage_e21_adc_b;
        d->last_adc_c = stage_e21_adc_c;
        d->last_floating_adc = stage_e21_floating_adc;
        d->last_positive_adc = stage_e21_positive_adc;
        d->last_negative_adc = stage_e21_negative_adc;
        d->dma_irq_count_delta = stage_e21_dma_fdt_count - live_sector_dma_fdt_start;
        d->ch4_event_count_delta = stage_e21_tmr1_ch4_event_count - live_sector_ch4_event_start;
        d->zc_state_at_timeout = (int)zc_arm_state;
        stage_e21_first_timeout_diag_valid = 1;

        capture_tmr1_regs(&stage_e21_tmr1_regs_at_timeout);
        stage_e21_tmr1_regs_at_timeout_valid = 1;
      }

      consecutive_fault_count++;
      check_fault_stop();

      /* Stage E21: 500ms-since-handover cap, checked here too so a
       * run stuck waiting through a long sector still stops on time. */
      if (stage_e21_running) {
      ...
      if (stage_e21_running) arm_timeout_watchdog();
      return;
    }

    /* TMR3_PURPOSE_COMMUTATION: this overflow IS the ZC-scheduled
     * commutation. */
    uint32_t prev_step_start = stage_e21_step_start_us;
    uint32_t prev_step_index = stage_e21_step_index; /* the sector that is ENDING -- apply_step() below overwrites stage_e21_step_index */
    int next = (int)((stage_e21_step_index + 1u) % 6u);

    apply_step(next, g_duty_ccr);
    stage_e21_step_count++;
    stage_e21_closed_loop_step_count++;

    /* Stage E21 instrumentation: capture the FIRST closed-loop
     * commutation's state, immediately after apply_step() -- proof of
     * what apply_step() actually reset for the new sector. */
    if (stage_e21_closed_loop_step_count == 1u && !stage_e21_first_closed_loop_snapshot_valid) {
      stage_e21_commutation_snapshot_t *snap = &stage_e21_first_closed_loop_snapshot;
      snap->step_index = stage_e21_step_index;
      snap->expected_dir = expected_dir[stage_e21_step_index];
      snap->pos_phase = stage_e21_positive_phase;
      snap->neg_phase = stage_e21_negative_phase;
      snap->floating_phase = stage_e21_floating_phase;
      ...
      snap->dma_fdt_count = stage_e21_dma_fdt_count;
      stage_e21_first_closed_loop_snapshot_valid = 1;

      capture_reg_snapshot(&stage_e21_regs_after_commutation);
      stage_e21_regs_after_commutation_valid = 1;

      capture_tmr1_regs(&stage_e21_tmr1_regs_after_commutation);
      stage_e21_tmr1_regs_after_commutation_valid = 1;

      stage_e21_cval_progression.cval_0 = TMR1->cval;
      stage_e21_cval_progression.cval_0_timestamp_us = TMR2->cval;
    }

    uint32_t now_us = stage_e21_step_start_us; /* just set by apply_step() */
    uint32_t period_us = (uint32_t)(uint16_t)(now_us - prev_step_start);
    if (period_us < stage_e21_sector_period_us_min) stage_e21_sector_period_us_min = period_us;
    if (period_us > stage_e21_sector_period_us_max) stage_e21_sector_period_us_max = period_us;
    stage_e21_sector_period_us_sum += period_us;
    last_valid_sector_period_us = period_us;

    /* Stage E21: per-step + overall sector-period stats, filed under
     * the step that just ended. */
    stat_update(&stage_e21_sector_period_by_step[prev_step_index], period_us);
    stat_update(&stage_e21_sector_period_overall, period_us);

    /* Stage E21: complete the sector-log entry for the sector that
     * just ended, using the pending_* values the DMA IRQ stashed at
     * the confirming ZC (before this commutation was even armed). */
    if (stage_e21_sector_log_count < SECTOR_LOG_SIZE) {
      stage_e21_sector_entry_t *se = &stage_e21_sector_log[stage_e21_sector_log_count];
      ...
      se->scheduled_delay_us = stage_e21_last_arm_log.requested_delay_us;
      ...
      stage_e21_sector_log_count++;
    }

    /* Stage E21: bounded test window -- 60 closed-loop commutations
     * or 500ms since handover, whichever first, stopping the same safe
     * way a real fault would. */
    update_elapsed_since_handover();
    if (stage_e21_closed_loop_step_count >= TEST_MAX_CLOSED_LOOP_COMMUTATIONS) {
...
void ADC1_IRQHandler(void)
{
  if (adc_interrupt_flag_get(ADC1, ADC_CCE_FLAG) != RESET) {
    ...
    stage_e21_adc_conversion_count++;
  }
}
...
void DMA1_Channel1_IRQHandler(void)
{
  if (dma_interrupt_flag_get(DMA1_FDT1_FLAG) != RESET) {
    ...
    stage_e21_dma_fdt_count++;

    /* Stage E21 instrumentation: the first FDT IRQ after entering
     * closed loop -- does the hardware auto-reload/re-arm look healthy
     * right after this one successful transfer? */
    if (stage_e21_mode == MODE_CLOSED_LOOP &&
        stage_e21_regs_after_commutation_valid &&
        !stage_e21_regs_after_first_dma_irq_valid) {
      capture_reg_snapshot(&stage_e21_regs_after_first_dma_irq);
      stage_e21_regs_after_first_dma_irq_valid = 1;

      /* Stage E21: cval_1, the second point of the non-blocking
       * progression check -- same IRQ, no busy-wait added. */
      stage_e21_cval_progression.cval_1 = TMR1->cval;
      stage_e21_cval_progression.cval_1_timestamp_us = TMR2->cval;
      stage_e21_cval_progression.valid = 1;
    }
    ...
    /* Stage E21: runs on EVERY closed-loop sample, independent of
     * blanking/zc_locked -- see file header. */
    if (stage_e21_running && !stage_e21_aligning && stage_e21_mode == MODE_CLOSED_LOOP) {
      alllow_update(a, b, c);
    }

    if (stage_e21_running && !stage_e21_aligning) {
      uint16_t v = vals[stage_e21_floating_phase];
      uint16_t pos_v = vals[stage_e21_positive_phase];
      uint16_t neg_v = vals[stage_e21_negative_phase];

      stage_e21_floating_adc = v;
      stage_e21_positive_adc = pos_v;
      stage_e21_negative_adc = neg_v;

      int32_t diff = (int32_t)v - (int32_t)pos_v / 2;
      stage_e21_floating_diff = diff;
      ...
      if (blank_scans_remaining) {
        ...
        /* Stage E21 instrumentation: live per-sector accumulators. */
        ...
        if (trace_active_group >= 0 &&
            stage_e21_step_trace_fill_count[trace_active_group] < STEP_TRACE_LEN) {
          uint32_t idx = stage_e21_step_trace_fill_count[trace_active_group];
          stage_e21_step_trace[trace_active_group][idx].positive_adc = pos_v;
          stage_e21_step_trace[trace_active_group][idx].negative_adc = neg_v;
          stage_e21_step_trace[trace_active_group][idx].floating_adc = v;
          stage_e21_step_trace_fill_count[trace_active_group] = idx + 1u;
          if (idx + 1u >= STEP_TRACE_LEN) {
            stage_e21_step_trace_captured[trace_active_group] = 1;
            trace_active_group = -1;
          }
        }
        ...
        if (confirmed_dir) {
          ...
          uint32_t now_us = TMR2->cval;
          uint32_t delay_us = (uint32_t)(uint16_t)(now_us - stage_e21_step_start_us);
          int matched = (confirmed_dir == expected_dir[stage_e21_step_index]);

          if (stage_e21_mode == MODE_RAMP || stage_e21_mode == MODE_HOLD) {
            if (matched) {
              stage_e21_consecutive_correct++;
            } else {
              stage_e21_consecutive_correct = 0;
            }

            if (stage_e21_mode == MODE_HOLD &&
                stage_e21_consecutive_correct >= HANDOVER_CONSECUTIVE_REQUIRED) {
              ...
              stage_e21_mode = MODE_CLOSED_LOOP;
              stage_e21_handover_success = 1;
              stage_e21_handover_step_index = stage_e21_step_index;

              /* Stage E21: start the bounded-test-window clock. */
              stage_e21_handover_timestamp_us = now_us;
              last_time_check_us = now_us;
              elapsed_since_handover_us = 0;

              /* Stage E21: stash this (handover) sector's log values
              ...
              stage_e21_sector_period_us_min = 0xffffffffu;
              stage_e21_sector_period_us_max = 0;
              stage_e21_sector_period_us_sum = 0;
              stage_e21_scheduled_delay_us_min = 0xffffffffu;
              stage_e21_scheduled_delay_us_max = 0;
              stage_e21_scheduled_delay_us_sum = 0;

              schedule_next_commutation(delay_us);
              stage_e21_scheduled_delay_us_min = delay_us;
              stage_e21_scheduled_delay_us_max = delay_us;
              stage_e21_scheduled_delay_us_sum = delay_us;

              /* Stage E21: the handover ZC's delay counts too. */
              stat_update(&stage_e21_zc_delay_by_step[stage_e21_step_index], delay_us);
              stat_update(&stage_e21_zc_delay_overall, delay_us);
            }
          } else if (stage_e21_mode == MODE_CLOSED_LOOP) {
            stage_e21_zc_count++;

            if (matched) {
              /* Stage E21: stash this sector's log values for the
              ...
              if (delay_us < stage_e21_scheduled_delay_us_min) stage_e21_scheduled_delay_us_min = delay_us;
              if (delay_us > stage_e21_scheduled_delay_us_max) stage_e21_scheduled_delay_us_max = delay_us;
              stage_e21_scheduled_delay_us_sum += delay_us;

              /* Stage E21: per-step + overall ZC-to-commutation (30-
              ...
              stage_e21_polarity_error_count++;
              consecutive_fault_count++;
              check_fault_stop();
...
int main(void)
{
  ...
  tim1_pins_to_af();
  stage_e21_aligning = 1;
  stage_e21_running = 1;
  stage_e21_mode = MODE_RAMP;
  stage_e21_step_index = 0;
  apply_step(0, g_duty_ccr);
  tmr_output_enable(TMR1, TRUE);
  tmr_counter_enable(TMR1, TRUE);
  delay_us(ALIGN_DURATION_MS * 1000u);
  stage_e21_aligning = 0;

  ramp_bucket_index = 0;
  start_ramp_bucket(0);
  stage_e21_step_count = 1;
